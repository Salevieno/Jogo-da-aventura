...
public abstract class LiveBeing implements Drawable
{
	...
	protected static final Clip HIT_SOUND = MusicManager.fileToClip(new File(Path.MUSIC + "16-Hit.wav").getAbsoluteFile()) ;	
	...
	public static GameMap calcNewMap(Point2D.Double pos, Directions dir, GameMap currentMap)
	{
		...
		return GameMap.getAllMaps().get(newMapID) ;		
	}
	...
	public void activateSpellCounters()
	{
		for (Spell spell : spells)
		{
			...
			if (this instanceof Player & job == 3 & spell.getId() == 114)
			{
				...
				{
					Buff.getAllBuffs().get(Game.getPet().getAlchBuffId()).apply(-1, spells.get(10).getLevel(), Game.getPet()) ;
