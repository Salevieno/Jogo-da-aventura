...
public class DrawFunctions
{
	...
	public void DrawOptionsWindow(Point NPCPos, Font font, int selOption, String[] options, Image NPCimage, Color color)
	{
		...
		for (int i = 0 ; i <= options.length - 1 ; i += 1)
		{
			...
			if (i == selOption)
			{
				...
				DP.DrawText(textPos, "TopLeft", stdAngle, options[i], font, ColorPalette[5]) ;
			}
			else
			{
				DP.DrawText(textPos, "TopLeft", stdAngle, options[i], font, color) ;	
			}
		}
	}
	public void DrawSpeech(Point Pos, String text, Font font, Image NPCimage, Image SpeakingBubble, Color color)
	{
		...
		{
			DP.DrawImage(SpeakingBubble, new Point(Pos.x + ImageL, Pos.y), stdAngle, new Scale(1, 1), new boolean[] {true, false}, "BotCenter", 1) ;
		}
		else
		{
			DP.DrawImage(SpeakingBubble, Pos, stdAngle, new Scale(1, 1), new boolean[] {false, false}, "BotCenter", 1) ;
		}
	...
	public void DrawWindowArrows(Point Pos, int L, int SelectedWindow, int MaxWindow)
	{
		...
		if (0 < SelectedWindow)
		{
			...
			DP.DrawImage(ArrowIconImage, LeftArrowPos, stdAngle, new Scale(-1, -1), "Center") ;
			DP.DrawText(LeftArrowPos, "TopRight", stdAngle, Player.ActionKeys[1], font, ColorPalette[5]) ;			
		}
		if (SelectedWindow < MaxWindow)
		{
			Point RightArrowPos = new Point(Pos.x + (int)(0.5 * L), Pos.y) ;
			DP.DrawImage(ArrowIconImage, RightArrowPos, stdAngle, new Scale(1, -1), "Center") ;
			DP.DrawText(RightArrowPos, "TopRight", stdAngle, Player.ActionKeys[3], font, ColorPalette[5]) ;		
		}
	}
	public void DrawOrganogram(int[] Sequence, Point Pos, int sx, int sy, Size size, String[][] Text1, String[] Text2, Icon SlotIcon, Font font, Color[] TextColor, Point MousePos)
	{
		...
		for (int row = 0 ; row <= Sequence.length - 1 ; row += 1)
		{
			for (int col = 0 ; col <= Sequence[row] - 1 ; col += 1)
			{
				...
				{
					DP.DrawTextUntil(new Point(slotCenter.x, slotCenter.y - IconH / 2 + 5 + textrow * textsy), "TopCenter", stdAngle, Text1[textrow][id], font, TextColor[id], 20, MousePos) ;
				}
				DP.DrawText(new Point(slotCenter.x, slotCenter.y + size.y / 3), "Center", stdAngle, Text2[id], font, TextColor[id]) ;							
				id += 1 ;
			}
		}
	}
	...
	public void DrawLoadingGameScreen(Player player, Pet pet, Map<String, String[]> allText, Icon[] icons, int SlotID, int NumberOfUsedSlots, Image GoldCoinImage)
	{
		...
		DP.DrawText(new Point((int)(0.5*screenSize.x), (int)(0.05*screenSize.y)), "Center", stdAngle, "Slot " + (SlotID + 1), font, ColorPalette[5]) ;
	...
	public void DrawEmptyLoadingSlot(int SlotID, int NumSlots)
	{
		...
		DP.DrawText(new Point(WindowPos[0].x + screenSize.x / 6, WindowPos[0].y + screenSize.y / 4), "Center", stdAngle, "Slot " + String.valueOf(SlotID + 1) + " is empty", new Font("SansSerif", Font.BOLD, 20), ColorPalette[5]) ;
	...
	public void DrawTime(Sky sky)
	{
		...
		DP.DrawText(new Point(0, (int) (0.99*screenSize.y)), "BotLeft", stdAngle, (int)(24*time) + ":" + (int)(24*60*time % 60), font, ColorPalette[5]) ;
	}
	...
	public void DrawFabBook(Image BookImage, Items[] items, int SelectedPage, int[][] Ingredients, int[][] Products, Point MousePos)
	{
		...
		int MaxTextL = 10 ;
		DP.DrawImage(BookImage, Pos, stdAngle, new Scale(1, 1), new boolean[] {false, false}, "Center", 1) ;
		DP.DrawText(new Point(Pos.x - 3*L/8, Pos.y - H/5 - sy/4), "BotLeft", stdAngle, "Ingredients:", Titlefont, ColorPalette[5]) ;
		DP.DrawText(new Point(Pos.x + 3*L/8, Pos.y - H/5 - sy/4), "TopRight", stdAngle, "Products:", Titlefont, ColorPalette[5]) ;		
		for (int j = 0 ; j <= Ingredients[SelectedPage].length - 1 ; ++j)
		{
			if (-1 < Ingredients[SelectedPage][j])
			{
				...
				{//
					DP.DrawTextUntil(new Point(Pos.x - 3*L/8, Pos.y - H/5 + IngredientsCont*sy + sy/2), "BotLeft", stdAngle, items[Ingredients[SelectedPage][j]].getName(), font, ColorPalette[5], MaxTextL, MousePos) ;
				//}
				IngredientsCont += 1 ;
			}
		}
		for (int j = 0 ; j <= Products[SelectedPage].length - 1 ; ++j)
		{
			if (-1 < Products[SelectedPage][j])
			{
				...
				{//
					DP.DrawTextUntil(new Point(Pos.x + 3*L/8, Pos.y - H/5 + ProductsCont*sy + sy/2), "TopRight", stdAngle, items[Products[SelectedPage][j]].getName(), font, ColorPalette[5], MaxTextL, MousePos) ;
				//}
				ProductsCont += 1 ;
			}
		}
	...
	public void DrawDamageAnimation(Point Pos, int damage, String effect, int counter, int duration, int aniStyle, Color color)
	{
		...
		{
			DP.DrawText(new Point(Pos.x + dx, Pos.y - dh), "Center", stdAngle, String.valueOf(UtilG.Round(damage, 1)) + "!", font, color) ;
		}
		else if (effect.equals("Miss"))
		{
			DP.DrawText(new Point(Pos.x + dx, Pos.y - dh), "Center", stdAngle, "Miss", font, color) ;		
		}
		else if (effect.equals("Block"))
		{
			DP.DrawText(new Point(Pos.x + dx, Pos.y - dh), "Center", stdAngle, "Block", font, ColorPalette[5]) ;	
		}
		else
		{
			DP.DrawText(new Point(Pos.x + dx, Pos.y - dh), "Center", stdAngle, String.valueOf(UtilG.Round(damage, 1)), font, color) ;	
	...
	public void DrawSkillNameAnimation(Point Pos, String SkillName, Color color)
	{
		...
		DP.DrawText(Pos, "Center", stdAngle, SkillName, font, color) ;
	}
