...
public class Opening
{
	...
	private static Image openingGif ;
	...
	public Opening()
	{
    	...
    	OPbuttons[0] = new Icon(0, "Port", new Point(Game.getScreen().getSize().width - 50, 30), null, ButtonPort, ButtonPortSelected) ;
    	OPbuttons[1] = new Icon(1, "En", new Point(Game.getScreen().getSize().width - 0, 30), null, ButtonEn, ButtonEnSelected) ;
    	OPbuttons[2] = new Icon(2, "New game", new Point(Game.getScreen().getSize().width / 2 - 80, Game.getScreen().getSize().height / 4), null, ButtonNewGame, ButtonNewGameSelected) ;
    	OPbuttons[3] = new Icon(3, "Load game", new Point(Game.getScreen().getSize().width / 2 + 80, Game.getScreen().getSize().height / 4), null, ButtonLoadGame, ButtonLoadGameSelected) ;
    	OPbuttons[4] = new Icon(4, "Male", new Point(Game.getScreen().getSize().width / 2 - 50, Game.getScreen().getSize().height / 4), null, null, null) ;
    	OPbuttons[5] = new Icon(5, "Female", new Point(Game.getScreen().getSize().width / 2 + 50, Game.getScreen().getSize().height / 4), null, null, null) ;
    	OPbuttons[6] = new Icon(6, "Baixo", new Point(Game.getScreen().getSize().width / 2 - 100, Game.getScreen().getSize().height / 4), null, null, null) ;
    	OPbuttons[7] = new Icon(7, "M�dio", new Point(Game.getScreen().getSize().width / 2 + 0, Game.getScreen().getSize().height / 4), null, null, null) ;
    	OPbuttons[8] = new Icon(8, "Alto", new Point(Game.getScreen().getSize().width / 2 + 100, Game.getScreen().getSize().height / 4), null, null, null) ;
    	OPbuttons[9] = new Icon(9, "Cavaleiro", new Point(Game.getScreen().getSize().width / 2 - 200, Game.getScreen().getSize().height / 4), null, null, null) ;
    	OPbuttons[10] = new Icon(10, "Mago", new Point(Game.getScreen().getSize().width / 2 - 100, Game.getScreen().getSize().height / 4), null, null, null) ;
    	OPbuttons[11] = new Icon(11, "Arqueiro", new Point(Game.getScreen().getSize().width / 2 + 0, Game.getScreen().getSize().height / 4), null, null, null) ;
    	OPbuttons[12] = new Icon(12, "Animal", new Point(Game.getScreen().getSize().width / 2 + 100, Game.getScreen().getSize().height / 4), null, null, null) ;
    	OPbuttons[13] = new Icon(13, "Ladr�o", new Point(Game.getScreen().getSize().width / 2 + 200, Game.getScreen().getSize().height / 4), null, null, null) ;
	...
	public int Run(String[] text, String action, Point MousePos, Music music, Animations Ani, DrawingOnPanel DP)
	{
		Font font = new Font(Game.MainFontName, Font.BOLD, 14) ;
		int ScreenW = Game.getScreen().getSize().width, ScreenH = Game.getScreen().getSize().height ;
		...
		// draw background
		DP.DrawImage(BackgroundImage, new Point(0, 0), 0, new Scale(1, 1), AlignmentPoints.topLeft) ;		
		...
		if (step == 0)
		{
			if (music.isOn())
			{
				Music.PlayMusic(music.getMusicIntro()) ;
			}
			step += 1 ;
		}
		if (step == 1)
		...
		else if (step == 2)
		{
			...
			DP.DrawText(new Point((int)(0.5*ScreenW) + 20, (int)(0.25*ScreenH)), AlignmentPoints.center, Textangle, text[1], font, TextColor) ;
			if (action.equals("Enter"))
			{
				Typing.LiveTyping(new Point((int)(0.4*ScreenW), (int)(0.3*ScreenH)), Textangle, action, Largefont, TextColor, DP) ;
			...
			{
				playerInfo[0] = Typing.LiveTyping(new Point((int)(0.4*ScreenW), (int)(0.3*ScreenH)), Textangle, action, Largefont, TextColor, DP) ;
			}
		}
		else if (step == 3)
		{
		...
		else if (step == 5)
		{
			...
			DP.DrawText(new Point((int)(0.35*ScreenW), (int)(0.1*ScreenH)), AlignmentPoints.bottomLeft, Textangle, text[5], font, TextColor) ;		
			for (int i = 0 ; i <= 5 - 1 ; i += 1)
			{
				Point Pos = new Point((int) (0.01 * ScreenW + i * sx), (int) (0.28 * ScreenH)) ;
				DP.DrawFitText(new Point(Pos.x + 5, Pos.y + 5), sy, AlignmentPoints.topLeft, text[11 + i], smallfont, TextL, TextColor) ;	
			}
