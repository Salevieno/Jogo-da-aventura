...
public class NPCsMethods
{
	...
	public void Contact(Player player, Pet pet, CreatureTypes[] creatureTypes, Creatures[] creatures, Spells[] skills, Maps[] maps, NPCs npc, Quests[] quest, Point MousePos, boolean TutorialIsOn, Image CoinIcon, Animations Ani, DrawFunctions DF)
	{
		...
		if (Choice.equals("Escape") & 0 < Menu[npc.getID()])
		{
			Menu[npc.getID()] = 0 ;
			for (int i = 0 ; i <= SelectedWindow.length - 1 ; i += 1)
			{
				SelectedWindow[i] = 0 ;
			}
		}
		...
		{
			Bank(Choice, player, npc, CoinIcon, DF) ;
		}
		...
		if (npc.getName().equals("Master") | npc.getName().equals("Mestre"))
		{
			Master(Choice, player, pet, skills, npc, MousePos, DF) ;
		}
	...
	public String ChoicesMenu(String playerAction, NPCs npc, String[] choiceOption, DrawFunctions DF)
	{
		...
		selChoice = Uts.MenuSelection(Player.ActionKeys[0], Player.ActionKeys[2], playerAction, selChoice, choiceOption.length - 1) ;
	...
	/*public int Sale(String Choice, Player player, NPCs npc, Items[] items, int[] MousePos, Image CoinIcon, DrawFunctions DF)
	{
		...
		} else if (Menu[npc.getID()] == 1)
		{
			...
			{
				Menu[npc.getID()] = Shopping(Choice, player, npc, items, "Selling", ItemsOnSale, MousePos, CoinIcon, DF) ;
			}
		} else if (Menu[npc.getID()] == 2)
		{
			DF.DrawSpeech(npc.getPos(), AllText[TextCat][2], NPCTextFont, npc.getImage(), SpeakingBubbleImage, npc.getColor()) ;
			ItemsOnSale = Utg.ArrayWithValuesGreaterThan(ItemsOnSale, 0) ;
			Menu[npc.getID()] = Shopping(Choice, player, npc, items, "Buying", ItemsOnSale, MousePos, CoinIcon, DF) ;
	...
	public int Master(String Choice, Player player, Pet pet, Spells[] skills, NPCs npc, Point MousePos, DrawFunctions DF)
	{
		...
		} else if (Menu[npc.getID()] == 1)
		{
			...
			{
				DF.DrawSpeech(npc.getPos(), AllText[TextCat][4], NPCTextFont, npc.getImage(), SpeakingBubbleImage, npc.getColor()) ;
				//DF.DrawChoicesWindow(npc.getPos(), NPCTextFont, new String[] {"(1) " + AllText[AllTextCat[5]][2*player.getJob() + 1], "(2) " + AllText[AllTextCat[5]][2*player.getJob() + 2]}, npc.getImage(), npc.getColor()) ;
				if (Choice.equals("1"))
				{ 
					player.setProJob(1) ;
					Menu[npc.getID()] = 3 ;
			...
			else
			{
				...
				if(Choice.equals("Enter") | Choice.equals("MouseLeftClick"))
				{
					if (Uts.SpellIsAvailable(player, skills, SelectedSkill) & 0 < player.getSkillPoints() & player.getSpell()[SelectedSkill] < skills[SelectedSkill].getMaxLevel())
					{
		...
		{
			DF.DrawSpeech(npc.getPos(), AllText[TextCat][5] + " " + AllText[AllTextCat[5]][player.getProJob() + 2*player.getJob()] + "!", NPCTextFont, npc.getImage(), SpeakingBubbleImage, npc.getColor()) ;
		}
		return Menu[npc.getID()] ;
	}
